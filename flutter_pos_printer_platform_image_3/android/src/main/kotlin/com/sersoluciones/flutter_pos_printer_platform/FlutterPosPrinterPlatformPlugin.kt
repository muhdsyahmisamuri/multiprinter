...
//                Log.d("bluetooth", "envia bt: ${String(readBuf)}")
    ...
    override fun onDetachedFromEngine(@NonNull binding: FlutterPlugin.FlutterPluginBinding) {
        ...
        // (onAttachedToActivity may not have been called in background execution contexts)
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        ...
        when {
            ...
            call.method.equals("printBytes") -> {
                // Handle both byte[] and ArrayList<Int> from Flutter
                val bytesArg: Any? = call.argument("bytes")
                val bytes: ArrayList<Int>? = when (bytesArg) {
                    is ByteArray -> ArrayList(bytesArg.map { it.toInt() and 0xFF })
                    is ArrayList<*> -> @Suppress("UNCHECKED_CAST") (bytesArg as ArrayList<Int>)
                    is List<*> -> ArrayList(bytesArg.filterIsInstance<Number>().map { it.toInt() })
                    else -> null
                }
