...
//                }
    ...
    fun init(reactContext: Context?) {
        ...
        // Create PendingIntent with proper flags for USB permission
        mPermissionIndent = if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
            // FLAG_MUTABLE + FLAG_UPDATE_CURRENT is required for USB permission to receive device extras
        ...
        // Register receiver with RECEIVER_NOT_EXPORTED for Android 13+ (API 33)
        if (android.os.Build.VERSION.SDK_INT >= 33) {
            mContext!!.registerReceiver(mUsbDeviceReceiver, filter, 4) // 4 = RECEIVER_NOT_EXPORTED
        } else {
