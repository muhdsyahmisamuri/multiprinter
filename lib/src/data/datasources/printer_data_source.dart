...
class PrinterDataSource {
  ...
  /// Print to TCP/Network printer using stored socket connection
  /// This allows true parallel printing to multiple network printers
  Future<void> _printToTcpPrinter(
    List<int> bytes,
    PrinterDeviceModel printer,
  ) async {
    try {
      // Get the stored socket connection
      Socket? socket = _tcpConnections[printer.id];

      // If no connection exists, try to connect first
      if (socket == null) {
        await _connectTcpPrinter(printer);
        socket = _tcpConnections[printer.id];
      }

      if (socket == null) {
        throw PrintJobException(
          message: 'Failed to get TCP connection for ${printer.name}',
          code: 'TCP_NO_CONNECTION',
        );
      }
      ...
      // Small delay to ensure printer receives all data
      await Future.delayed(const Duration(milliseconds: 100));

    } on SocketException catch (e) {
      // Connection might be broken, remove it so next attempt will reconnect
      _tcpConnections.remove(printer.id);
      ...
      throw PrintJobException(
        message: 'Failed to print to network printer ${printer.address.address}: $e',
